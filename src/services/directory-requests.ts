...
const columns = "id,profile_id,kind,message,reply_email,status,created_at,updated_at,can_review";
...
export const directoryRequestResponse = (input: unknown) => {
    ...
    return { id: row.id, profileId: row.profile_id, kind: row.kind, message: row.message, replyEmail: row.reply_email,
        status: row.status, createdAt: row.created_at, updatedAt: row.updated_at, canReview: row.can_review, emailDelivery: "not_configured" as const };
};
...
export const reviewDirectoryRequest = async (token: string, id: string, status: "resolved" | "rejected", client = createUserScopedSupabaseClient(token)) => {
    ...
    fail(error);
    return getDirectoryRequest(token, z.uuid().parse(data), client);
};
