...
Response fields: `id, profileId, kind, message, replyEmail, status, createdAt, updatedAt, canReview, emailDelivery`. `status` starts `pending`. `emailDelivery` is always `not_configured`; no email provider is configured, no email is queued, and no email delivery is claimed. The requests and in-app notifications are persisted. UI success copy should say "Request submitted" and optionally "Available in the recipient's in-app requests inbox".
...
All writes use scoped authenticated RPCs. Table RLS protects reads; authenticated callers have no direct insert/update/delete grant. Definer RPCs independently validate authentication, inputs, target visibility and recipient review permission. The safe read view uses `security_invoker=true`, so table RLS remains effective. No service-role client or external email/send API is used.
